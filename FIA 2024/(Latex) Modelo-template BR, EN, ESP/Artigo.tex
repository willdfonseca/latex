...
Ele foi adaptado do modelo da \href{https://revista.acustica.org.br}{Revista Acústica e Vibrações}, sendo de uso para o XIII~Congresso Ibero-Americano de Acústica (FIA 2024)
%
...
Este documento já conta com a formatação de estilos personalizados para a elaboração do artigo. O autor pode, portanto, utilizar este arquivo como modelo para essa finalidade. Serão disponibilizados modelos (\textit{templates}) em Microsoft Word (\texttt{.docx}) e \LaTeX\xspace (\texttt{.tex}). Esta versão também está disponível no \href{https://www.overleaf.com/read/tjbcfwbtfdtz\#869489}{Overleaf} e no \href{https://github.com/willdfonseca/latex}{GitHub} --- sendo ainda compatível com Windows, Mac e Linux. 
...
\begin{enumerate} \itemsep=2pt
	...
	\item A primeira página deve conter (para as línguas portuguesa e espanhola) título, autores, filiações, resumo, palavras-chave, \textit{title}, \textit{abstract} e \textit{keywords}. Submissões em inglês podem conter apenas autores, filiações, \textit{title}, \textit{abstract} e \textit{keywords} (sem versões em português ou espanhol).
	\item O texto deve ser escrito em língua culta vigente.
	\item O número de páginas deve ser no mínimo 6 e no máximo 15, contando da página que contém o título, até o final das referências (incluindo apêndices, se houver).
...
O resumo do artigo poderá ter entre 150 e 200 palavras (em tipografia de 11~pt). O resumo, palavras-chave, \textit{title}, \textit{abstract} e \textit{keywords} constituem a primeira página do artigo, é recomendado não se estender para a segunda página. 
...
Seguindo o resumo, o autor deve listar até cinco palavras-chave (evite colocar as mesmas palavras que formam o título do artigo). O texto do artigo deve começar propriamente após os \textit{keywords}.
...
O trabalho completo deve conter de 6 a 15 páginas, contando da  página que contém o título e o final da lista de referências. São admitidos apêndices, depois das referências, desde que estes não ultrapassem 15 páginas no total. 

Como forma de otimizar ao máximo o conteúdo de cada página, as figuras, tabelas, quadros e códigos devem ser apresentados ao longo do corpo do texto (são aceitas figuras lado a lado).
...
Texto subscrito e sobrescrito somente será em itálico se for correspondente a alguma variável pertinente. Caso seja um ``nome complementar'', o texto deve ser colocado em pé, por exemplo, $P\txu{total}$ corresponde à pressão total em Pa, ou ainda $S\txup{tri}$ corresponde à área do triângulo em cm$^2$. 
%
Contudo, ao se referir a uma variável, como $i$, deve-se escrever, por exemplo: ``o somatório foi calculado considerando $P_i$ até a $i$-ésima pressão final, correspondente a 256''.
...
As figuras, tabelas, quadros e códigos\footnote{A distribuição deste \textit{template} de \LaTeX\xspace inclui o pacote \ttc{Codes2Latex.sty}, que habilita possibilidades para documentação de códigos genéricos e nas linguagens Matlab, Fortran, Python, LabView e Latex de forma organizada (observe o Código~\ref{code.matlalatex}) --- o pacote está ainda em desenvolvimento.} deverão ser centralizados e numerados sequencialmente (vide exemplo nas Figuras~\ref{fig:beamforming} e \ref{subfig.exemplo}; Tabela~\ref{tab.exemplo}; Quadro~\ref{quad.exemplo} e Código~\ref{code.matlalatex}). Busque utilizar figuras e gráficos em que seu conteúdo possa ser completamente compreendido. 
...
\begin{figure}[!ht]
  ...
  \centering
	\subfloat[Legenda da Figura~\protect\subref*{fig.figA}.]{\label{fig.figA}
            \makebox[.46\columnwidth]{\includegraphics[height=35mm,page=48]{example-image-duck}} } % makebox ajuda a organizar figs lado a lado
	\quad
...
O evento aceitará \textbf{submissões originais} (isto é, ainda não publicadas) de pesquisas científicas e aplicações de engenharia, arquitetura, áudio, física, matemática, fonoaudiologia e áreas (e subáreas) afins. Assim, sugerem-se os seguintes tipos de documento:
%
\begin{itemize}[topsep=0ex] \itemsep=2pt
	\item \textbf{Artigos técnicos e aplicados} (\textit{Technical and applied papers}):
	apresentam material original a partir de aplicações de técnicas conhecidas e/ou em desenvolvimento. Devem descrever métodos aplicados que estejam em conformidade com normativas e/ou que apresentem resultados relevantes. É fundamental que esses artigos despertem o interesse de pesquisadores e profissionais na área proposta.
	
	\item \textbf{Artigos científicos} (\textit{Scientific papers}): 
	incluem material original (ideias, modelos, experimentos \etc) inédito, que contribui significativamente para o avanço do conhecimento científico no tema abordado. O conteúdo deve estabelecer uma conexão com o \textit{estado da arte} existente na literatura publicada.

	\item \textbf{Artigos de revisão} (\textit{Review papers}):
	abordam o \textit{estado da arte} sobre o tema em questão, elucidando desde conceitos básicos até aspectos mais complexos. Esse tipo de submissão deve ser abrangente em relação à literatura existente, cobrindo amplamente ideias, modelos, experimentos \etc, já desenvolvidos, mesmo que discordem da visão do autor. É crucial que o tópico seja de interesse para a comunidade científica.
\end{itemize}
...
\begin{enumerate}[topsep=0ex] \itemsep=0.0pt
    \item Acústica Arquitetônica e de Edifícios
    \item Acústica Biomédica e Bioacústica
    \item Acústica Computacional
		\vspace{-0.25em}
    \begin{enumerate}[noitemsep,topsep=0ex] 
        \item Imageamento Acústico e Acústica Virtual/Auralização
    \end{enumerate}
    \item Acústica Estrutural e Vibrações
    \item Acústica Física e Ultrassom
		\vspace{-0.25em}
    \begin{enumerate}[noitemsep,topsep=0ex]
        \item Metamateriais Estruturados para o Controle de Ruído e Vibrações
    \end{enumerate}
    \item Acústica Forense
    \item Acústica Musical
    \item Acústica Psicológica e Fisiológica
    \item Acústica Subaquática
    \item Áudio Profissional e Eletroacústica
    \item Educação em Acústica
    \item Paisagens Sonoras e Ecoacústica
    \item Processamento de Sinais em Acústica
    \item Ruído Ambiental, Industrial e Ocupacional
\end{enumerate}
...
O trabalho deve ser estruturado; portanto, sugerem-se os seguintes itens:
%
\begin{itemize}[noitemsep,topsep=0ex] \itemsep=3pt
	...
	\item Referências: apresentar as referências bibliográficas citada no texto.
\end{itemize}
%
Não é preciso necessariamente existir seções com estes nomes. A organização é também dependente do tipo do artigo.
Outros elementos pós-textuais como apêndices são opcionais.
...
Dependendo do contexto, o nome do autor pode ou não ser escrito, observe alguns exemplos a seguir: 
%
\begin{itemize}[noitemsep,topsep=0ex] \itemsep=4pt
	...
	\item ``... \txtcite{Fonseca-2013} demonstrou o cálculo de difração para superfícies cilíndricas~\cite{Fonseca-2013}.''
\end{itemize}
%
...
\begin{itemize}[noitemsep,topsep=0ex] \itemsep=8pt
	...
	\item ``Trabalhos em temas de acústica \cite{Mareze-2017,Oppenheim-2010,Muller-2001, Mareze-2019, jasa:2022eac}.''
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Submissão de artigos}

Os artigos completos deverão ser enviados pelo sistema próprio do congresso, disponível no site\linebreak \url{https://www.fia2024.cl}, dentro dos prazos estabelecidos. Detalhes acerca de registro autor participante podem ser consultados também no site do evento ou com a comissão organizadora.


É responsabilidade dos autores a preparação e envio dos artigos em seu formato final. Por esse motivo, pede-se que verifiquem com atenção a formatação de seus artigos, especialmente gráficos e fotos, quanto à legibilidade e qualidade digital (e para impressão). \textbf{Os artigos deverão ser enviados em formato PDF (com tamanho máximo de 16~Mb).} 
...
O modelo de \LaTeX\xspace (\texttt{.tex}) foi escrito em codificação UTF8, assim é compatível com Windows, Mac, Linux e \href{https://www.overleaf.com/read/tjbcfwbtfdtz\#869489}{Overleaf}\footnote{\url{https://www.overleaf.com/read/tjbcfwbtfdtz\#869489}.}. Pode ser usado livremente para a elaboração dos artigos.

O modelo de \texttt{.docx} foi criado em Microsoft Word 2016 e, com isso, suas funcionalidades de espaçamento e configurações são garantidas para essa versão. Todas eles estão disponíveis com links no \href{https://www.fia2024.cl}{site do evento} (ou \href{https://github.com/willdfonseca/latex}{neste repositório}).

O autor deste texto e dos modelos é o professor William D'Andrea Fonseca, da Engenharia Acústica (EAC) da Universidade Federal de Santa Maria (UFSM), Brasil.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Considerações finais}

Este \textit{artigo modelo} teve como objetivo elencar e esclarecer as diretrizes para a submissão de trabalhos ao XIII~Congresso Ibero-Americano de Acústica. Este documento serve como um guia prático, podendo ser utilizado como modelo ao substituir seu conteúdo conforme necessário.
